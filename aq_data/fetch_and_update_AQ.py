...
# Get pm25 values
try:
    pm25_penr_value = penr["data"]["iaqi"]['pm25']['v']
except:
    pm25_penr_value = None
try:
    pm25_patu_value = patu["data"]["iaqi"]['pm25']['v']
except:
    pm25_patu_value = None
try:
    pm25_taka_value = taka["data"]["iaqi"]['pm25']['v']
except:
    pm25_taka_value = None

# Calculate the maximnum value
pm25 = max(pm25_penr_value,
             pm25_patu_value,
             pm25_taka_value,
             1)
...
for i in (1, 2, 3, 4, 5):
    # Extract pm25 and no2 from a wide normal distribution
    noisy_pm25 = random.lognormvariate(math.log(pm25), 0.1)
    noisy_no2 = random.lognormvariate(math.log(no2), 0.1)
    level_pm25 = min(round(10 * (max(0, noisy_pm25 - 5) / 50)), 10)
    level_no2 = min(round(5 * (max(0, noisy_no2 - 0) / 20)), 5)
    print("NO2 level ", level_no2)
    print("PM2.5 level ", level_pm25)
    ...
    # devices watching
    result = SendCommand('agent/metadata/setvalue',
                         {'agentid': treeID, 'key': 'particlespeed', 'value': level_pm25})
    time.sleep(8)
print("--- %s seconds ---" % (time.time() - start_time))
# Update thingspeak channel
options = {'api_key': writekey, 'field1': pm25, 'field2': no2}
